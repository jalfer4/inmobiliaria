...
namespace Inmobiliaria_.Net_Core.Models
{
    public class RepositorioAlquiler : RepositorioBase, IRepositorioAlquiler
    {
        ...
        string sql = $"INSERT INTO Alquileres ( Descripcion,FechaAlta , FechaBaja , Monto , InmuebleId , InquilinoId, GaranteId ) " +
                    $"VALUES ('{entidad.Descripcion}','{entidad.FechaAlta}', '{entidad.FechaBaja}','{entidad.Monto}','{entidad.InmuebleId}','{entidad.InquilinoId}','{entidad.GaranteId}')";
        ...
        {//AlquilerId ,FechaAlta , FechaBaja , Monto , InmuebleId , InquilinoId, GaranteId  
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                ...
                using (SqlCommand command = new SqlCommand(sql, connection))
                {
                    ...
                    command.Parameters.Add("@GaranteId", SqlDbType.Int).Value = alquiler.GaranteId;
                    command.CommandType = CommandType.Text;
                    connection.Open();
                    res = command.ExecuteNonQuery();
                    connection.Close();
